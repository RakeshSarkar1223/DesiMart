...
export default function CartSidebar({ isOpen, onClose }) {
  ...
  const [isCheckingOutForm, setIsCheckingOutForm] = useState(false);
  ...
  // Handle Order Placement to Java Backend
      ...
      <div className="relative w-full max-w-md h-full bg-obsidian-dark/95 border-l border-obsidian-border shadow-2xl flex flex-col justify-between z-10 animate-slide-in-right glass-panel">
        ...
        {isCheckingOutForm ? (
          <form onSubmit={handlePlaceOrder} className="flex-grow flex flex-col justify-between overflow-hidden">
            {/* Form Fields */}
            <div className="flex-grow overflow-y-auto px-6 py-4 space-y-4 custom-scrollbar text-left">
              <h3 className="text-xs font-bold text-slate-200 uppercase tracking-wider mb-2 border-b border-obsidian-border/50 pb-2">Shipping Information</h3>
              <div>
                <label className="block text-[10px] font-semibold text-slate-400 mb-1.5 uppercase tracking-wider">Full Name</label>
                <input 
                  type="text" 
                  required
                  placeholder="Rakesh Sarkar"
                  value={address.fullName}
                  onChange={(e) => setAddress({...address, fullName: e.target.value})}
                  className="w-full rounded-lg border border-obsidian-border bg-obsidian-dark px-3 py-1.5 text-xs text-slate-200 outline-none focus:border-neon-cyan"
                />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[10px] font-semibold text-slate-400 mb-1.5 uppercase tracking-wider">Phone No</label>
                  <input 
                    type="text" 
                    required
                ...
                <div>
                  <label className="block text-[10px] font-semibold text-slate-400 mb-1.5 uppercase tracking-wider">Pin Code</label>
                  <input 
                    type="text" 
                    required
                    placeholder="700001"
              ...
              <div>
                <label className="block text-[10px] font-semibold text-slate-400 mb-1.5 uppercase tracking-wider">Street Address</label>
                <input 
                  type="text" 
                  required
                  placeholder="123 Luxury Avenue, Salt Lake"
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[10px] font-semibold text-slate-400 mb-1.5 uppercase tracking-wider">City</label>
                  <input 
                    type="text" 
                    required
                    placeholder="Kolkata"
                ...
                <div>
                  <label className="block text-[10px] font-semibold text-slate-400 mb-1.5 uppercase tracking-wider">State</label>
                  <input 
                    type="text" 
                    required
              ...
              <h3 className="text-xs font-bold text-slate-200 uppercase tracking-wider mt-4 mb-2 border-b border-obsidian-border/50 pb-2">Payment Method</h3>
                  ...
                  <button
                    ...
                    className={`py-2 rounded-lg border text-xs font-bold transition-all cursor-pointer ${
                      paymentMethod === method 
                        ...
                        : 'border-obsidian-border text-slate-400 bg-obsidian-dark hover:border-slate-700'
                    }`}
                  >
                    {method}
                  </button>
            ...
            <div className="border-t border-obsidian-border bg-obsidian-darkest/90 p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3.5 pt-2">
                <button
                  type="button"
                  onClick={() => setIsCheckingOutForm(false)}
                  className="w-full py-3 rounded-xl border border-obsidian-border hover:bg-slate-900/60 text-xs font-semibold text-slate-400 hover:text-slate-200 transition-colors cursor-pointer text-center"
                >
                  Back to Cart
                </button>
                ...
                <button
                  ...
                  className="w-full py-3 rounded-xl bg-gradient-to-r from-neon-cyan to-neon-blue text-xs font-bold text-obsidian-darkest glow-btn-cyan cursor-pointer text-center flex items-center justify-center space-x-2"
                >
                  {checkingOut ? (
                    <>
            ...
            {cart && Array.isArray(cart.items) && cart.items.length > 0 && (
              <div className="border-t border-obsidian-border bg-obsidian-darkest/90 p-6 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3.5 pt-2">
                  ...
                  <button
                    disabled={checkingOut}
                    onClick={() => setIsCheckingOutForm(true)}
